...
function createTask(fromMagento, options) {
    ...
    return DB.TaskList.find({
        ...
        if (obj.momentTime) {
            ...
            taskObject.tags = [ config.tags.scheduledButNotConfirmed ]
            ...
            taskObject.tags = [ config.tags.notScheduled ]
        }
        ...
        return ASANA.teams.users(config.teams.gns).then(team => {
            ...
            if (obj.momentTime) {
                ...
                let sectionShift = ASANA.sections.addTask(
                    config.projects.main.sections.scheduledButNotConfirmed, {
                        task: taskData.gid
                    }
                )
            ...
            if (fromMagento.data.customer_email && fromMagento.data.customer_email.toLowerCase() !== 'sayhi@greyandsanders.com') {
                addContact({
                    ...
                    lastName: fromMagento.data.customer_lastname,
                    phone: fromMagento.data.customer_telephone,
